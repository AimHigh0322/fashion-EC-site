...
class ApiService {
  ...
  async getProduct(id: string) {
    ...
    try {
      ...
      // Handle 404 errors gracefully (product doesn't exist) - return undefined data instead of error
      if (response.status === 404) {
        return { data: undefined };
      }

      // For other errors, use the standard request method
      if (!response.ok) {
  ...
  // Get order by Stripe session ID (for webhook-based flow)
  async getOrderBySessionId(sessionId: string) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/checkout/order-by-session?session_id=${sessionId}`,
        {
          method: "GET",
          headers,
        }
      ...
      // Handle 404 gracefully - order not created yet by webhook (expected during polling)
      if (response.status === 404) {
        return { data: undefined };
      }
