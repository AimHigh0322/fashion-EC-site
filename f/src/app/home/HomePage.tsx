...
export const HomePage = () => {
  ...
  const filteredRecommendedProducts = useMemo(() => {
    ...
    return filtered;
  }, [recommendedProducts, selectedCategoryId, searchQuery]);
            ...
            Promise.all(
              viewedIds.slice(0, 8).map(async (productId) => {
                try {
                  ...
                  // Silently handle errors (product doesn't exist, network error, etc.)
                  return null;
                }
              })
            ).then((products) => {
            ...
            <div className="w-full lg:flex-[2] relative bg-gray-100 overflow-hidden">
              ...
              {thumbnailImages.length > 1 && (
                <>
                  ...
                  >
                    <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5 text-gray-800 transition-colors duration-200 group-hover:text-white cursor-pointer" />
                  </button>
                  <button
                  ...
                  >
                    <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-gray-800 transition-colors duration-200 group-hover:text-white cursor-pointer" />
                  </button>
                </>
              )}
            </div>
                ...
                >
                  {filteredRecommendedProducts.map((product) => (
                    <Link
                      ...
                      className="group flex-shrink-0 w-[calc((100%-48px)/5)] sm:w-[calc((100%-64px)/5)] min-w-[180px] sm:min-w-[200px] bg-white border border-gray-200 overflow-hidden shadow-sm flex flex-col cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 hover:-translate-y-1"
                    >
                      <div className="relative overflow-hidden">
                        <img
                          src={product.image}
