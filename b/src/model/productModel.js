...
async function createProduct(productData, userId) {
  ...
  try {
    ...
    const productUrl = `http://manabou.co.jp/${sku}`;
...
async function updateProduct(productId, productData, userId) {
  ...
  try {
    ...
    if (productData.sku !== undefined && productData.sku !== currentSku) {
      ...
      updateValues.push(`http://manabou.co.jp/${productData.sku}`);
      ...
      if (!product[0]?.product_url && currentSku) {
        updateFields.push("product_url = ?");
        updateValues.push(`http://manabou.co.jp/${currentSku}`);
      }
    }
...
async function getProductById(productId) {
  ...
  // Get attributes (use LEFT JOIN to handle cases where attribute_definitions might not exist)
    ...
    for (const productData of productsData) {
      try {
        ...
        if (existing.length > 0) {
          ...
          const productUrl = productData.product_url || `http://manabou.co.jp/${productData.sku}`;
